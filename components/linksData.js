const data = [
    {
        title: "For Digital",
        list: [
            {
                title: "Web", list: [
                    "framer",
                    "webflow",
                    "custom website",
                    "eCommerece",
                    "web funnel",
                    "landing pages",
                    "PWA development",
                    "SEO",
                ]
            },
            {
                title: "Mobile", list: [
                    "adolo apps",
                    "native apps",
                    "cross-platform apps",
                    "enterprise mobile",
                ]
            },
            {
                title: "software", list: [
                    "custom software",
                    "SAAS software",
                    "enterprise software",
                    "web-based software",
                ]
            },
            {
                title: "UI/UX design", list: [
                    "UI/UX design",
                    "concepting & prototyping",
                    "gamification & interactivity",
                    "A/B testing",
                ]
            },
            {
                title: "tech consulting", list: [
                    "digital transformation",
                    "AI strategy & redesign",
                    "digital commerece strategy",
                    "digital funnel optimization",
                    "fast MVP",
                    "devOps strategy",
                    "application modernization",
                ]
            }
        ]
    }
    ,
    {
        title: "for multimedia",
        list: [
            {
                title: "visiual media", list: [
                    "photography",
                    "videography",
                    "motion graphics",
                    "generative AI",
                ]
            },

            {
                title: "VTuber creation", list: [
                    ""
                ]
            },

            {
                title: "3d modeling & rendering", list: [
                    "product visiualization",
                    "character modeling",
                    "conceptual visiualization",
                    "3D environment",
                ]
            },

            {
                title: "multimedia consulting", list: [
                    "unreal engine",
                    "virtual production",
                    "AR, VR & MR (XR)",
                    "motion capture",
                    "full-funnel media",
                    "gamification & interactivity",
                ]
            },
        ]

    },
    {
        title: "For creative",
        list: [
            { title: "digital advertising", list: [] },
            {
                title: "creative campaigns", list: [
                    "creative production",
                    "UGC campaigns",
                    "rebranding campaigns",
                    "IMC campaigns",
                ]
            },
            {
                title: "social media", list: [
                    "social media strategy",
                    "content strategy & creation",
                    "content creator management",
                ]
            },
            {
                title: "brand strategy & visiual identity", list: [
                    "brand strategy & narrative",
                    "visiual identity",
                    "brand audit & guidelines",
                ]
            },

            {
                title: "creative consulting", list: [
                    "trend forecasting & analysis",
                    "full-funnel media",
                    "venture consulting",
                    "content marketing strategy",
                ]
            },
        ],
    },


    {
        title: "For Talent",
        list: [
            {
                title: "seeking representation?", list: [
                    "content creator",
                    "live streamer",
                    "VTuber",
                ]
            },
            {
                title: "clubsoda collective", list: [
                    "specialist",
                    "agency",
                ]
            },
            {
                title: "consulting", list: [
                    "tech consulting",
                    "multimedia consulting",
                    "creative consulting",
                    "venture consulting",
                ]
            },
            {
                title: "for brands (coming soon)",
                list: [],
            }

        ]
    },

];
export default data;
